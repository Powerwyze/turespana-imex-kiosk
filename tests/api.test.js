...
for (const count of [1, 2, 3]) {
  test(`exactly ${count} nearest guest(s) approved; one generation and one independent check`, async () => {
    ...
    assert.equal(payload.images[2].image_url,'data:image/jpeg;base64,'+readFileSync('public/assets/examples/regional-clothing.jpg').toString('base64'));
    assert.match(payload.prompt,/lower-left panel labelled Madrid/);
    assert.match(payload.prompt,/head-to-toe/);
