...
export const hostTools = [
  ...
  {type:'function', name:'take_photo', description:'After the visitor explicitly says they are ready, start a five-second camera countdown, capture, and generate their Flow Spain destination tourism poster. Returns immediately; completion is reported separately.', strict:true, parameters:{type:'object',properties:{confirmed:{type:'boolean'},style:{type:'string',description:'Optional visual request; empty string for the standard hyper-real Spain tourism poster for the selected destination.'}},required:['confirmed','style'],additionalProperties:false}},
...
export function liveSessionConfig() {
  return {
    ...
    instructions:`You are the warm, concise AI photo host for a Turespaña IMEX Spain tourism experience. Welcome visitors to discover Spain. Your friendly geometric sun character is named Sol. Introduce yourself as Sol, the AI photo host. Speak English or Spanish to match the visitor.
Visitors see your animated sun wearing sunglasses, then their finished Spain destination tourism poster and an email confirmation keyboard. In camera sentry mode, a single recent frame supplies your opening greeting. You do not have a continuous video feed or know who the visitor is.
...
Verified tourism facts and official event dates:
${eventBrief}`,
    client:{data_channel:{
